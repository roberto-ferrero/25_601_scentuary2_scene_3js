//import gsap from "gsap"
import * as THREE from 'three'

import { Water } from 'three/examples/jsm/objects/Water2.js'

class Pools{
    constructor (obj){
        // console.log("(Pools.CONSTRUCTORA): ", obj)
        this.app = obj.app
        this.project = obj.project
        this.stage = obj.stage
        this.scenario = obj.scenario
        this.sunPosition = obj.sunPosition
        this.sunColor = obj.sunColor
        this.parent3D = obj.parent3D
        //-----------------------------

        //-----------------------------
        this.itemId = "poolwalls"
        this.mesh = this.stage.get_mesh_from_GLB_PROJECT(this.itemId)
        this.texture = this.stage.loader.get_texture("poolwalls")
        this.texture.flipY = false;
        //--
        // this.texture_ao = this.stage.loader.get_texture("walls_ao")
        // this.texture_ao.flipY = false;
        //--
        const marbleMaterial = new THREE.MeshStandardMaterial({
            map: this.texture, // Use the loaded texture
            color: this.scenario.BESE_MARBEL_COLOR, // Ivory base color
            // color: 0xff0000, // Ivory base color
            // aoMap: this.texture_ao,
            // aoMapIntensity: 0.5,
            roughness: 0.4,   // Moderate roughness for a soft shine
            metalness: 0.0,   // Non-metallic
            // emissive: 0xbab4b1, // Ivory base color
            // emissiveIntensity: 0.2, // Soft glow
        });
        this.mesh.material = marbleMaterial
        this.mesh.castShadow = true;
        this.mesh.receiveShadow = true;
        // console.log("mesh",this.mesh);
        this.parent3D.add(this.mesh)
        //-----------------------------

        const params = {
            color: '#ffffff',
            scale: 4,
            flowX: 1,
            flowY: 1
        };
        //-----------------------------
        // this.mesh = this.stage.get_mesh_from_GLB_PROJECT(this.itemId)

        
        const waterNormals = this.stage.loader.get_texture("waternormals");
        waterNormals.wrapS = waterNormals.wrapT = THREE.RepeatWrapping;
        //---
        //const seaGeometry = new THREE.PlaneGeometry(100, 400, 10, 100) // Replace size if needed
        this.pool1_imported_mesh = this.stage.get_mesh_from_GLB_PROJECT("pool3");
        this.pool1_geo = this.pool1_imported_mesh.geometry;
        console.log("this.pool1_geo: ", this.pool1_geo);

        this.pool1_mesh = new Water(this.pool1_geo, {
            color: 0x001e0f,        // base color
            scale: 4.0,             // wave scale
            flowDirection: new THREE.Vector2(1, 1),
            textureWidth: 1024,
            textureHeight: 1024,
            // // color: 0x001e0f,
            // scale: 4,
            // // flowDirection: new THREE.Vector2(1, 1),
            // textureWidth: 1024,
            // textureHeight: 1024,
            // // textureWidth: 512,
            // // textureHeight: 512,
            // flowDirection: new THREE.Vector2(1, 1),
            // // waterNormals: new THREE.TextureLoader().load(
            // //     'https://threejs.org/examples/textures/waternormals.jpg',
            // //     texture => texture.wrapS = texture.wrapT = THREE.RepeatWrapping
            // // ),
            waterNormals:waterNormals,
            normalMap0: this.stage.loader.get_texture("normalMap0"),
            normalMap1: this.stage.loader.get_texture("normalMap1"),
            // // sunDirection: this.sunPosition,
            // sunDirection: new THREE.Vector3(0, 1, 0), // Assuming the sun is directly above
            // sunColor: this.sunColor,
            // waterColor: 0xf5a863,
            // distortionScale: 1,
            // fog: this.app.scene.fog !== undefined
        })
        
        
        console.log("flowDirection: ", this.pool1_mesh.material.uniforms.flowDirection);
        // this.mesh.rotation.x = -Math.PI / 2
        // this.mesh.rotation.z = Math.PI
        this.pool1_mesh.position.set(-6,  0.25, 0)
        // this.pool1_mesh.position.set(-6,  0.25, -4.5)
        console.log("this.pool1_mesh.position: ", this.pool1_mesh.position);
        // this.mesh.rotation.z = -Math.PI / 2
        this.parent3D.add(this.pool1_mesh)
        
        this.pool1_mesh.material.uniforms.reflectivity.value = 0


    }
    //----------------------------------------------
    // PUBLIC:
    update_RAF (){
        
    }
    //----------------------------------------------
    // EVENTS:

    //----------------------------------------------
    // PRIVATE:

    //----------------------------------------------
    // AUX:

  
}
export default Pools